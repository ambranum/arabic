...
def main():
    ...
    for bid, (en, test) in BOOKS.items():
        ...
        with open(os.path.join(OUT_DIR, bid + '.js'), 'w', encoding='utf-8') as bf:
            # Name the edition this book actually came from. The line was hardcoded to Van
            # Dyck, so all 66 Hebrew files -- the Leningrad Codex and Delitzsch -- shipped
            # claiming to be a 19th-century Arabic translation. A provenance line that is wrong
            # is worse than none: it is the one thing in a generated file a reader trusts.
            bf.write('// GENERATED by pipeline/bible.py -- %s, public domain.\n' % edition[bid])
    ...
    with open(INDEX, 'w', encoding='utf-8') as f:
        ...
        f.write('// %s: %s. PUBLIC DOMAIN. English (ESV) is fetched at\n'
                % (paths.LANG, ', '.join(sorted(set(edition.values())))))
