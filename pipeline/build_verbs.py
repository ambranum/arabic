...
# Full paradigms — sound & hollow Form I (the shipped waves). The engine is DERIVED by
# rule and verified against the Lingualism reference (pipeline/verify_conjugation.py:
# sound 98.7%, hollow 99.6%; residual is optional variation, not error). Verbs that don't
# parse cleanly keep just their principal parts — we never attach a paradigm we can't stand behind.
from conjugate import conjugate, conjugate_hollow
def paradigm(x):
    if x['form'] != 1 or not x['pres']:
        return None
    if x['weak'] == 'sound':
        return conjugate(x['root'], x['past']['caphi'], x['pres']['caphi'])
    if x['weak'] == 'hollow':
        return conjugate_hollow(x['root'], x['past']['caphi'], x['pres']['caphi'])
    return None
...
_nhollow = sum(1 for v in data['verbs'] if 'conj' in v and v['weak'] == 'hollow')
print('\nfull paradigms attached (sound + hollow Form I): %d verbs (%d hollow)' % (_nconj, _nhollow))
