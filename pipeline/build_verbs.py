...
# Full paradigms — sound Form I only (this wave). The engine is DERIVED by rule and
# verified against the Lingualism reference (pipeline/verify_conjugation.py, 98.7%; the
# residual is optional variation, not error). Verbs that don't parse as a clean sound
# triliteral keep just their principal parts — we never attach a paradigm we can't stand behind.
from conjugate import conjugate
def paradigm(x):
    if x['form'] != 1 or x['weak'] != 'sound' or not x['pres']:
        return None
...
print('\nfull paradigms attached (sound Form I): %d verbs' % _nconj)
